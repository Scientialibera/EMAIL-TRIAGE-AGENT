...
$zipPath = Join-Path $env:TEMP "email-triage-agent-deploy.zip"
if (Test-Path $zipPath) { Remove-Item $zipPath }

$stagingDir = Join-Path $env:TEMP "email-triage-agent-staging"
...
# 2. Seed mailbox config blob
$mailboxConfigFile = "deploy/assets/config/mailbox_config.json"
if (Test-Path $mailboxConfigFile) {
    Write-Step "Seeding mailbox config blob."
    Upload-BlobFromFile -AccountName $storageAcct -Container $configContainer -BlobName "mailbox_config.json" -FilePath $mailboxConfigFile
} else {
    Write-Step "No mailbox_config.json found -- skipping seed."
}

# 3. Seed prompts
Write-Step "Seeding prompt blobs."
$promptsDir = "deploy/assets/prompts"
if (Test-Path $promptsDir) {
    foreach ($promptFile in (Get-ChildItem -Path $promptsDir -Filter "*.txt")) {
        Upload-BlobFromFile -AccountName $storageAcct -Container $promptsContainer -BlobName $promptFile.Name -FilePath $promptFile.FullName
    }
}
...
if (Test-Path $manifestDir) {
    ...
    $manifestOutputDir = Join-Path $env:TEMP "email-triage-manifest-staging"
