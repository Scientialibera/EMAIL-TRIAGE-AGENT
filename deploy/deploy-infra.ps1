...
$cosmosAcct   = Select-Value $config.naming.cosmos_account_name (Normalize-CosmosName -Value $prefix)
$docIntelName = Select-Value $config.naming.doc_intel_name (Normalize-CogName -Prefix "docintel" -Value $prefix)
...
$deployDocIntel   = [bool]$config.doc_intelligence.deploy_doc_intel
...
foreach ($container in @($configContainer, $promptsContainer)) {
...
Write-Step "Ensuring Cosmos DB account '$cosmosAcct'."
$cosmosExists = az cosmosdb list --resource-group $rg --query "[?name=='$cosmosAcct'] | length(@)" -o tsv
if ($cosmosExists -eq "0") {
    az cosmosdb create `
      --resource-group $rg --name $cosmosAcct --location $location `
      --capabilities EnableServerless `
      --default-consistency-level Session | Out-Null
}

$dbName = $config.cosmos.database_name
$containerName = $config.cosmos.container_name
$partitionKey = $config.cosmos.partition_key

$dbExists = az cosmosdb sql database list --resource-group $rg --account-name $cosmosAcct --query "[?name=='$dbName'] | length(@)" -o tsv
if ($dbExists -eq "0") {
    az cosmosdb sql database create --resource-group $rg --account-name $cosmosAcct --name $dbName | Out-Null
}

$colExists = az cosmosdb sql container list --resource-group $rg --account-name $cosmosAcct --database-name $dbName --query "[?name=='$containerName'] | length(@)" -o tsv
if ($colExists -eq "0") {
    az cosmosdb sql container create `
      --resource-group $rg --account-name $cosmosAcct --database-name $dbName `
      --name $containerName --partition-key-path $partitionKey | Out-Null
}
...
if ($deployDocIntel) {
    Write-Step "Ensuring Document Intelligence '$docIntelName'."
    $diExists = az cognitiveservices account list --resource-group $rg --query "[?name=='$docIntelName'] | length(@)" -o tsv
    if ($diExists -eq "0") {
        az cognitiveservices account create `
          --name $docIntelName --resource-group $rg --kind FormRecognizer `
          --sku $config.doc_intelligence.sku --location $location `
          --custom-domain $docIntelName | Out-Null
    }
}

# 7. Entra App Registration
$entraAppName = Select-Value $config.entra.app_display_name "Email Triage Agent"
...
$docIntelEndpoint = ""
if ($deployDocIntel) {
    $docIntelEndpoint = az cognitiveservices account show --resource-group $rg --name $docIntelName --query properties.endpoint -o tsv
    $docIntelScope = az cognitiveservices account show --resource-group $rg --name $docIntelName --query id -o tsv
    Ensure-RoleAssignment -PrincipalId $webAppPrincipalId -Scope $docIntelScope -Role "Cognitive Services User"
}

$cosmosScope = az cosmosdb show --resource-group $rg --name $cosmosAcct --query id -o tsv
Ensure-RoleAssignment -PrincipalId $webAppPrincipalId -Scope $cosmosScope -Role "Cosmos DB Built-in Data Contributor"
...
$cosmosEndpoint = az cosmosdb show --resource-group $rg --name $cosmosAcct --query documentEndpoint -o tsv
...
$settings = @(
    ...
    "BLOB_PROMPTS_CONTAINER=$promptsContainer",
    "COSMOS_ENDPOINT=$cosmosEndpoint",
    "COSMOS_DATABASE=$($config.cosmos.database_name)",
    "COSMOS_CONTAINER=$($config.cosmos.container_name)",
...
Write-Output "Cosmos DB:            $cosmosAcct ($($config.cosmos.database_name)/$($config.cosmos.container_name))"
Write-Output "Web App:              $webAppName (https://$webAppName.azurewebsites.net)"
Write-Output "Azure OpenAI:         $openAiAcct"
Write-Output "Doc Intelligence:     $docIntelName"
